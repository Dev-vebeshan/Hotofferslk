...
if (isset($_POST['add'])) {
    ...
    // Fetch admin email
    $query = mysqli_query($con, "SELECT mail FROM user WHERE role='Admin' AND status='active' LIMIT 1");
    if (mysqli_num_rows($query) > 0) {
        $row = mysqli_fetch_array($query);
        $admin_mail = $row['mail'];
    } else {
        $admin_mail = ""; // Default if no admin is found
    }

    $insert = $con->prepare("INSERT INTO inbox (name, sender, receiver, title, message, created_at, status, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $insert->bind_param('ssssssis', $customer_name, $customer_mail, $admin_mail, $title, $text, $current_time, $status, $customer_role);
    ...
    if ($insert->execute()) {
        ...
        $message = '<p class="mt-4 text-gray-600">Message Successfully Sent</p>';
        $button = '<a href="message.php" class="bg-[#F97225] text-white rounded-lg hover:bg-[#fe8745] px-4 py-2 rounded-md transition duration-300">OK</a>';
    } else {
        $showModal = true;
        $img = '<img src="./assets/alarm.gif" alt="" style="width:50px; height:50px;">';
        $title = '<h2 class="text-xl font-semibold text-black">Error</h2>';
        $message = '<p class="mt-4 text-gray-600">Message Not Sent</p>';
        $button = '<button id="closeAlert" class="bg-yellow-500 text-white px-4 py-2 rounded-md transition duration-300">OK</button>';
    }
}
...
if (isset($_GET['uid'])) {
    ...
    $sql = mysqli_query($con, "UPDATE inbox SET status=0 WHERE receiver='$uid'");

    if ($sql) {
        $showModal = true;
        $img = '<img src="./assets/verified.gif" alt="" style="width:50px; height:50px;">';
        $title = '<h2 class="text-xl font-semibold text-black">Success</h2>';
        $message = '<p class="mt-4 text-gray-600">Status Successfully Updated</p>';
        $button = '<a href="message.php" class="bg-[#F97225] text-white rounded-lg hover:bg-[#fe8745] px-4 py-2 rounded-md transition duration-300">OK</a>';
    } else {
        $showModal = true;
        $img = '<img src="./assets/alarm.gif" alt="" style="width:50px; height:50px;">';
        $title = '<h2 class="text-xl font-semibold text-black">Error</h2>';
        $message = '<p class="mt-4 text-gray-600">Status Not Updated</p>';
        $button = '<button id="closeAlert" class="bg-yellow-500 text-white px-4 py-2 rounded-md transition duration-300">OK</button>';
    }
}
?>
...
<body class="bg-gray-100 min-h-screen flex flex-col">
          ...
          <div class="bg-white shadow-lg rounded-lg p-6">
            <section class="mb-4">
              <div class="mb-8">
                ...
                <p class="text-sm text-gray-500">Manage your Inbox messages here</p>
              </div>
             <div class="flex flex-col sm:flex-row justify-end items-center gap-3 mt-3 mb-3">
    <a class="px-4 py-2 bg-[#F97225] text-white text-center rounded-lg hover:bg-[#fe8745] flex items-center justify-center h-[42px] cursor-pointer w-full sm:w-auto" onclick="openModal()">
        + Add New
    </a>

    <a href="sent_message.php" class="px-4 py-2 bg-[#F97225] text-white text-center rounded-lg hover:bg-[#fe8745] flex items-center justify-center h-[42px] w-full sm:w-auto">
        Sent Messages
    </a>

    <a href="message.php?uid=<?php echo $customer_mail; ?>" class="text-center border border-green-600 px-4 py-2 rounded-lg text-green-600 hover:bg-green-100 transition flex items-center justify-center h-[42px] w-full sm:w-auto">
        Mark as Read
    </a>
</div>
  ...
  <div id="advertisementModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-2xl max-h-[90vh] p-6 rounded-lg shadow-lg overflow-y-auto">
      ...
      <div class="flex justify-between items-center border-b pb-3">
        <h4 class="text-xl font-semibold text-gray-700">New Message</h4>
        <button 
          onclick="closeModal()" 
      ...
      document.getElementById("advertisementModal").classList.add("hidden");
                  ...
                  <div class="tbl-content">
                    <table id="example">
                      <tbody>
                           <?php 
            $x=1;
            $sql=$con->prepare("SELECT * FROM inbox WHERE receiver=? ORDER BY id DESC");
            ...
            if($result->num_rows > 0)
            {
              while($row=$result->fetch_assoc())
              {
                ...
                if($row['status']===1)
            {
                $status='<span class="inline-flex items-center rounded-full bg-orange-100 text-orange-600 px-3 py-1 text-xs font-medium">Unread</span>';
            }
            else
            {
                $status='<span class="inline-flex items-center rounded-full bg-green-100 text-green-600 px-3 py-1 text-xs font-medium">Read</span>';
            }
                ?>
                       <tr>
                                <td><?php echo $x; ?></td>
                                <td><?php echo htmlspecialchars($row['title']); ?></td>
                                <td><textarea rows="3" readonly><?php echo htmlspecialchars($row['message']); ?></textarea></td>
  ...
  <!--                        <td><button class="text-blue-500 hover:underline" onclick="opendel('del<?php echo $del_id; ?>')">-->
  <!--<img src="./assets/img/delete-3-svgrepo-com.svg" width="20" style="filter: invert(33%) sepia(86%) saturate(6488%) hue-rotate(357deg) brightness(100%) contrast(104%);" alt="Edit Icon">-->
  <!--            </button></td>-->
...
<!--        <center><a href="message.php?delid=<?php echo $del_id; ?>" class="mt-5 px-4 text-sm py-2 bg-[#F97225] text-white rounded-lg hover:bg-[#fe8745]">Delete</a> <button onclick="closedel('del<?php echo $del_id; ?>')" class="mt-5 px-4 text-sm py-2 bg-black text-white rounded-lg hover:bg-black-400">Cancel</button></center>-->
<!--      </div>-->
<!--   </div>-->
<!--</div>-->
<!-- del model -->
