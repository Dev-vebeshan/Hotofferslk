...
if(isset($_GET['slug']))
{
    ...
    $find_cat=mysqli_query($con,"SELECT * FROM brands WHERE slug='$slug' ");
    if(mysqli_num_rows($find_cat) >0)
    {
        $cat_row=mysqli_fetch_array($find_cat);
        $brand_id=$cat_row['id'];
        $brand_name=$cat_row['name'];
        $brand_description=$cat_row['description'];
    }
    else
    {
...
<head>
  ...
  <meta name="description" content="Discover discounts from top brands on HotOffers.lk. Browse offers from leading brands across various categories and enjoy great savings!">
   ...
   <title><?php echo ucfirst($brand_name); ?></title>
...
<div style="height: 70vh; background-image: url('./assets/images/banner1.png'); background-size: cover; background-position: center;" class="position-relative w-100">
  <div class="position-absolute text-white d-flex flex-column align-items-start justify-content-center" style="top: 0; right: 0; bottom: 0; left: 0; ">
    ...
    text-transform: capitalize;
    ">
    <?php echo ucfirst($brand_name); ?>
</h3>
<p style="font-size:12px;"><?php echo ucfirst($brand_description); ?></p>
      </div>
    </div>
  </div>
</div>
...
$sql=$con->prepare("SELECT a.id AS Ads_ID,
    ...
    a.brand_id,
    a.category_id,
    a.status,AVG(r.rating) AS avg_rating, r.id, count(r.id) AS tot_rating, r.rating, b.id, b.name AS brand_name, c.id, c.name AS category_name
FROM advertisments AS a LEFT JOIN review AS r ON a.id = r.ads_id INNER JOIN brands AS b ON a.brand_id=b.id INNER JOIN category AS c ON a.category_id=c.id
WHERE 
    a.status = 'Active' AND a.brand_id=?
GROUP BY 
    a.id
ORDER BY 
    a.id DESC");
