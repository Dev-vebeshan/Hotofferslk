...
<head>
  ...
  <meta content="width=device-width, initial-scale=1.0" name="viewport">
   <title>Explore Categories | Best Deals in Sri Lanka | HotOffers.lk</title>
...
<div style="height: 70vh; background-image: url('./assets/images/banner3.png'); background-size: cover; background-position: center;" class="position-relative w-100">
    ...
    <div class="container">
      <div class="col-md-8 p-0">
       <h1 class="mobileh1" style="font-size: 2.5rem;
            font-weight: 700;
            margin-bottom: 20px;">Categories</h1>
        <div class="mt-5">
          <!-- hover background-color: white; color: black; -->
			 <p class="trending mt-2">Browse through our wide range of categories and uncover a world of options tailored to your needs and preferences! Whether you're searching for the latest fashion styles, high-quality electronics, home essentials, or beauty products, our extensive selection has something for everyone.</span></p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="container mt-3">
    ...
    <?php 
    $query=$con->prepare("SELECT * FROM category WHERE status=1 ORDER BY id DESC");
    $query->execute();
    ...
    if($result->num_rows > 0)
    {
      while($row=$result->fetch_assoc())
      {
        ...
        <div class="card-body">
          <a href="category.php?slug=<?php echo $row['slug']; ?>">
          <center> <img style="width: 30px; height:30px;" src="<?php echo $row['image']; ?>" alt="brand_img"></center>
